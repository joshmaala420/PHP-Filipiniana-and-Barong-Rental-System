...
// Modify your SQL query to filter by search term
$sql = "SELECT * FROM users WHERE full_name LIKE '%$searchTerm%' OR email LIKE '%$searchTerm%' OR phone LIKE '%$searchTerm%'";

$UsersRecords = mysqli_query($conn, $sql);
...
<div class="search-container" style="margin-bottom: 20px;">
    <form method="GET" action="">
        <input type="text" name="search" placeholder="Search users..." class="form-input" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
                ...
                <?php 
                    while($row = mysqli_fetch_assoc($UsersRecords)){
                     ...
                     <tr>
                        ...
                        <td><button class="btn btn-edit btn-success" type="button" onclick="openEditModal(<?php echo $row['id']; ?>, '<?php echo htmlspecialchars($row['full_name']); ?>', '<?php echo htmlspecialchars($row['email']); ?>', <?php echo htmlspecialchars($row['phone']); ?>, '<?php echo htmlspecialchars($row['password']); ?>')">Edit</button>
