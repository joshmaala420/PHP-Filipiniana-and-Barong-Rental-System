...
try {
    ...
    foreach ($cart_items as $cart_item_id) {
        $query = "SELECT cart.product_id, cart.size, products.prod_title, products.price, products.prod_img, cart.qty
                FROM cart
        ...
        if ($result && $row = mysqli_fetch_assoc($result)) {
            ...
            $size = $row['size'];
            $qty = $row['qty'];  // Retrieve the quantity from the cart for this product
            $item_total = $product_price * $qty;  // Calculate item total based on price * qty
            ...
            $insert_order = "INSERT INTO orders (product_id, user_id, product_name, product_image, transaction_no, size, qty, total, created_at)
                            VALUES ('{$row['product_id']}', '$user_id', '$product_name', '$product_image', '$transaction_no', '$size', '$qty', '$item_total', NOW())";
    ...
    $insert_to_pay_order = "INSERT INTO to_pay_orders (user_id, email, transaction_no, reference_no, qty, total, time_r, date_r, created_at, status)
                            VALUES ('$user_id', '$user_email', '$transaction_no', '$reference_no', '$total_qty', '$total_order_price', '$Time_r', '$Date_r', NOW(), 'Pending')";
