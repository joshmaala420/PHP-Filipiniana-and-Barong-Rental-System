...
$reviews_sql = "SELECT reviews.*, users.full_name FROM reviews LEFT JOIN users ON reviews.user_id = users.id WHERE reviews.product_id = ?";
...
<header>
    ...
    <?php
        ...
        <a href="index.php"><li>Home</li></a>      
            ...
            ?>
<!-- Dropdown for displaying cart products -->
<a href="cart.php" class="cart-icon" id="cartDropdown">
    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-cart-plus" viewBox="0 0 16 16">
        ...
        <div class="product-container">
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    $productId = $_SESSION['product_id'] = $row['id'];
                    ...
                    <div class="product-card" style="position: relative"
                        ...
                        data-product-description="<?php echo $productDescription; ?>"
                        data-product-price="<?php echo $productPrice; ?>"
                        data-product-qty="<?php echo $productQty; ?>"
                        data-reviews='<?php echo json_encode($reviews); ?>'>
                        ...
                        <h3 class="price-description">₱<?php echo $productPrice; ?></h3>
                    </div>
        ...
        <div class="dropdown-content" id="cartDropdownMenu">
    <?php
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<li>";
            echo "<img src='" . htmlspecialchars($row['product_image']) . "' alt='" . htmlspecialchars($row['product_name']) . "' class='cart-product-image' />";
    ...
    ?>
        ...
        <div id="productModal" class="modal-pro">
            <div class="modal-pro-content">
                <span class="close" onclick="closeModal()">&times;</span>

                <!-- Product Image Section with Rating -->
                ...
                <!-- Product Details and User Reviews -->
                <div class="modal-details">
                    <h2 id="modalProductName"></h2>
                    <p id="modalProductDescription"></p>
                    <p id="modalProductPrice" class="product-price"></p>
                    <p id="modalProductId"></p>
                    <p id="modalProductQty"></p>

                        <div class="size-options">
                        <span onclick="SelectedSize(event)" class="size-box">S</span>
                        <span onclick="SelectedSize(event)" class="size-box">M</span>
                        <span onclick="SelectedSize(event)" class="size-box">L</span>
                        <span onclick="SelectedSize(event)" class="size-box">XL</span>
                    </div>
                            <!-- Quantity Adjustment -->
                    <div class="quantity-container">
                        <button type="button" class="btn btn-outline-secondary" id="decreaseQty">-</button>
                        <input type="number" id="productQuantity" name="qty" value="1" min="1" class="button-btn" style="width: 60px; text-align: center;">
                        <button type="button" class="btn btn-outline-secondary" id="increaseQty">+</button>
                    </div>

                    <form action="add_to_cart.php" method="POST" onsubmit="return checkFormData()">
                        <input type="hidden" name="product_id" id="productIdInput">
                        <input type="hidden" name="size" id="sizeInput">
                        <input type="hidden" name="user_id" value="<?php echo isset($_SESSION['user_id']) ? $_SESSION['user_id'] : ''; ?>">
                        <input type="hidden" name="price" id="priceInput">
                        <input type="hidden" name="qty" id="quantityInput" value="1"><br><br>
                    ...
                    <div class="comments-section">
                        <h3>User Reviews</h3>
                        <div class="comments-scrollable" id="commentsList">
...
function openModal(productId, productName, productImage, productDescription, productPrice, productQty, reviews) {
    document.getElementById('modalProductName').innerText = productName;
    document.getElementById('modalProductImage').src = productImage || 'path/to/default-image.jpg';
    ...
    document.getElementById('modalProductQty').innerText = `Product Qty: ${productQty}`;

    document.getElementById('productModal').setAttribute('data-product-qty', productQty);
    ...
    if (reviews.length > 0) {
        reviews.forEach(review => {
            ...
            newComment.innerHTML = `
                <p class="username">${review.full_name || 'Anonymous User'}</p>
...
document.querySelectorAll('.product-card').forEach(card => {
    card.addEventListener('click', function() {
        ...
        const productQty = parseFloat(card.getAttribute('data-product-qty'));
        const reviews = JSON.parse(card.getAttribute('data-reviews'));

        openModal(productId, productName, productImage, productDescription, productPrice, productQty, reviews);
    });
});
...
let selectedSize = '';  // Variable to store selected size
    ...
    function SelectedSize(event) {
        ...
        sizeBoxes.forEach(box => {
            box.classList.remove('selected-Size');
        ...
        // Log the selected size to console for debugging
        console.log("Selected size: " + selectedSize);
    }
    // Function to check if both product_id and size are selected before submitting the form
    function checkFormData() {
    // Get the product quantity from the modal
    const productQty = parseInt(document.getElementById('productModal').getAttribute('data-product-qty'));

    // If the quantity is 0, prevent adding to the cart
    if (productQty === 0) {
...
document.getElementById("decreaseQty").addEventListener("click", function() {
    ...
    if (parseInt(qtyInput.value) > 1) {
        qtyInput.value = parseInt(qtyInput.value) - 1; // Decrease by 1
    }
    updateQuantityInput();
});
